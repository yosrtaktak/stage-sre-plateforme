...
```text
    (1)  DETECTER  ------  StackRox  ·  Prometheus
          |
          v
    (2)  TRIER  ---------  EPSS  ·  CISA KEV  ·  la charge tourne-t-elle ?
          |
          v
    (3)  PROPOSER  ------  le LLM redige  ·  l'allow-list tranche
          |
          v
    (4)  SE SOUVENIR  ---  registre : jamais reproposer ce qui fut refuse
          |
          v
    (5)  TRANSMETTRE  ---  pull request  ·  ou issue si l'agent refuse
          |
          v
   +=========================================================+
   ||                                                       ||
   ||   (6)  M E R G E   H U M A I N                        ||
   ||        le seul geste humain de toute la chaine        ||
   ||        6 gates CI  ·  CODEOWNERS  ·  branche protegee ||
   ||                                                       ||
   +=========================================================+
          |
          v
    (7)  DEPLOYER  ------  Argo CD
          |
          v
    (8)  CONFIRMER  ----- rescan : la violation a-t-elle disparu ?
          |
          +-------------> le verdict retourne au registre (4)
```
...
```text
 +------------------------------------------------------------------+
 |  APPLICATION      Online Boutique  ·  12 microservices            |
 |                   Istio ambient  ·  waypoint L7                   |
 +---------------------------------+--------------------------------+
                                   |  istio_requests_total
 +---------------------------------v--------------------------------+
 |  OBSERVABILITE    Prometheus  ->  SLI  ·  SLO  ·  burn rates      |
 |                   Grafana  ·  Loki  ·  Tempo  ·  Kiali            |
 |                   Alertmanager                                    |
 +---------------------------------+--------------------------------+
                                   |  alertes
 +---------------------------------v--------------------------------+
 |  SECURITE         StackRox Central  ->  admission + detection     |
 |                   Litmus Chaos  ->  injecte des pannes            |
 +---------------------------------+--------------------------------+
                                   |  violations
 +---------------------------------v--------------------------------+
 |  AGENT SRE                                                        |
 |                   contexte     EPSS · CISA KEV · charge active    |
 |                       |                                           |
 |                       v                                           |
 |                   HolmesGPT    le modele PROPOSE                  |
 |                       |                                           |
 |                       v                                           |
 |                 [ ALLOW-LIST ]  le code DECIDE   <-- garde-fou    |
 |                   /         \                                     |
 |          autorise/           
efuse                              |
 +----------------/-------------\-----------------------------------+
                 v               v
 +------------------------------------------------------------------+
 |  DEPOT GIT        pull request        issue                       |
 |                       |                                           |
 |                       v                                           |
 |                   6 gates CI   gitleaks · Trivy · CodeQL · cosign |
 |                       |                                           |
 |                       v                                           |
 |               [ ** MERGE HUMAIN ** ]             <-- garde-fou    |
 +---------------------------------+--------------------------------+
                                   |
 +---------------------------------v--------------------------------+
 |  GITOPS           Argo CD  ->  6 Applications  ->  cluster K3s    |
 +---------------------------------+--------------------------------+
                                   |
                                   +--> rescan : retour a l'agent
                                   +--> incidents : base neutre
                                        PostgREST · GoAlert · Slack
```
