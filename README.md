...
flowchart LR
    ...
    classDef humain fill:#fff3cd,stroke:#b8860b,stroke-width:3px,color:#5c4600
    classDef auto fill:#eef4fb,stroke:#3f6ea8,color:#1c3855
    class F humain
    class A,B,C,D,E,G,H auto
```
...
flowchart TB
    subgraph APP ["🛒 APPLICATION"]
        MS["Online Boutique — 12 microservices<br/>Istio ambient · waypoint L7"]
    end

    subgraph OBS ["📊 OBSERVABILITÉ"]
        PROM["Prometheus<br/>SLI · SLO · burn rates"]
        VIZ["Grafana · Loki · Tempo · Kiali"]
        AM["Alertmanager"]
    end

    subgraph SECU ["🛡️ SÉCURITÉ RUNTIME"]
        SR["StackRox Central<br/>admission + détection"]
        LIT["Litmus Chaos"]
    end
    ...
    subgraph DEPOT ["🔀 DÉPÔT GIT"]
        PR["pull request"]
        ISS["issue"]
        GATES["6 gates CI<br/>gitleaks · Trivy · CodeQL · cosign"]
        MERGE["★ merge humain ★"]
    end

    subgraph GO ["🚀 GITOPS"]
        ARGO["Argo CD — 6 Applications"]
        K3S["cluster K3s"]
    end

    subgraph INC ["📟 INCIDENTS"]
        NDB["base neutre<br/>PostgreSQL + PostgREST"]
        GOA["GoAlert"]
        SLK["Slack — war room"]
    end
    ...
    PROM --> VIZ
    SR --> CTX
    LIT -. injecte des pannes .-> MS
    CTX --> LLM --> ALLOW
    LEDG -. mémoire .-> ALLOW
    ALLOW -->|autorisé| PR
    ALLOW -->|refusé| ISS
    PR --> GATES --> MERGE --> ARGO --> K3S
    K3S -. déploie .-> MS
    K3S -. rescan .-> LEDG
    CTX --> NDB --> GOA
    CTX --> SLK
    ...
    classDef humain fill:#fff3cd,stroke:#b8860b,stroke-width:3px,color:#5c4600
    classDef garde fill:#e8f5e9,stroke:#2e7d32,stroke-width:2px,color:#1b5e20
    class MERGE humain
    class ALLOW garde
```
